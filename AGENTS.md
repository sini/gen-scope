...
| Computing the dirty set / change propagation for an incremental rebuild | `gen-memo` — "gen-memo — the incremental plane: a decision layer over the evaluator that never evaluates, only decides reuse". gen-scope supplies only the consumer-driven hooks: `evalWarm`'s `decision` (`isClean` + `reusable`) over a `prior` accessor, the `facade` the plane reads through, and the sealed context's `accessor.dependencies` together with the `trace` derived over it |
...
| Channels / dataflow riding over scopes | **`gen-view`, which inherited it — `gen-pipe` RETIRED as a library rather than moving as one.** ADR-0010 §3 retires gen-pipe into the movement vocabulary; twelve of its seventeen exports name gen-view constructs (the fourth destination §3 gained on 2026-08-20) and `sel` retires into `gen-select`. The gen-pipe repository orphans as reference under ADR-0031 §3's F3 pattern, off the `gen/lib/mkGenLibs.nix` roster and not a `gen` hub input. ★ Its old dependency line was `prelude+select+scope`, so THIS library was underneath it — and still is: a channel is now a named materialized query result over an authored scope graph, so the capability rides gen-scope through gen-view rather than through a library of its own |
...
`accessorRecord` keys, observed: `node`, `get`, `allNodes`, `allNodeIds`, `allNodesWhere`, `subtreeOf`, `nodesOfType`, `_walkFrom`, plus the plane interface — `facade`, `resolutional`, `served`, `structuralAttributes`, `decisionFindings`, `provenance`. Tier 1 is `node id` / `get id attrName`; the rest are Tier 2 materializers that force the tree.
...
**Declared-reads surface: there is NONE, and the absence is the design.** gen-scope has no `readsAttrs`; `git grep -n readsAttrs` over tracked files ⇒ no hits, and the same predicate hits `gen-resolve/lib` (see negative space). It has no read-set construct of its own either: **the body IS the read set**, so a read set declared beside a rule does not simplify away — it never exists (Van Gelder 1991 Def 3.3 and §8; Sagiv 1990 printed 664; Vogt 1989 Def 3.5 p. 139). Reads DERIVE from the graph's edges.

Where the read edges actually are, enumerated:

- **The declared relation** — `foldEquations`' `declaredDependencies` formal, sealed on the context and published as `accessor.dependencies`. This is a DEPENDENCY declaration, which is a different thing from a read declaration: it is what the gate runs over, complete at registration. It is node-level, the coarsening Knuth's `D(T)` undergoes when its attribute-instance vertices are projected onto positions; a *read* is attribute-level, and the granularity is what separates them.
- **The trace** — `ctx.trace.<id>.deps`, derived per node as `accessor.dependencies id` at seal time. It duplicates nothing: it is the declared relation, indexed. Tests: `test-trace-deps-are-the-accessor-dependencies`, `test-trace-deps-literal`, `test-trace-deps-empty-where-no-dependency-is-declared` (`ci/tests/fold-equations.nix`).
- **Evaluation order** — governed by neither. `eval` schedules nothing; Nix's laziness is the scheduler.
- **Memoization** — governed by neither. Neither the declared relation nor the trace runs through `get`, so neither reads nor populates any `_eval` cache.
- **Reuse admission** — governed by neither. `evalWarm` decides reuse from `decision.isClean id` and `decision.reusable id` only, and never for a structural attribute.
- **Reachability / error reporting** — governed by neither. Node resolution uses `roots`, `parseParent`, and the `children` / `derived-children` attributes.
- **What the evaluators accept** — neither takes a `declaredDependencies` argument (both patterns are closed; passing one is an arity error). The relation reaches the seal through `foldEquations` and nowhere else.

★ **WHAT IS CONTRACTED AND WHAT IS NOT, stated because the law names it as the discriminator.** `declaredDependencies` is REQUIRED and TOTAL — no default — so absence is refused by the evaluator naming the formal rather than silently meaning *this node depends on nothing*, which is the strongest claim available and was previously awarded to the caller who said nothing. What is still NOT contracted is the codomain and the coverage: no check refuses a declared endpoint naming an id outside the node set, and none refuses a body that reads a dependency it never declared — `accessor.dependencies`' own comment says the caller "must over-declare" and nothing enforces it. The relation is therefore complete at registration in FORM and unenforced in FACT, and a gate trusting it inherits that gap.
...
| `circular` | `{ carrier = { bottom, leq, height } } -> (self -> id -> prev -> next) -> self -> id -> value` — the carrier is REQUIRED and refused by name; the iteration bound is DERIVED as `height + 1` |
...
**Attribute-name contract** (consumed, not exported). `children` and `derived-children` are reserved attribute *names* that the evaluator special-cases; `imports` and `edges-<label>` are the names the resolution combinators read. None of the four is supplied for you — the consumer declares them all in `attributes`. ★ All four are STRUCTURAL and so never served from a prior evaluation. `imports` reaches both the resolver and the classifier from one binding (`lib/traversal-names.nix`), and the `edges-` prefix likewise: `lib/structural.nix` binds and exports it as `edgePrefix`, and `lib/resolve.nix` reads that binding where it builds an edge name rather than writing the string a second time. A second relation added to the vocabulary is reserved by construction, but a traversal writing its own string literal instead is NOT — a predicate over attribute names cannot read a caller's literals.
...
★★ **The answer's KEY SET is closed, and the claim payload's is not.** Every field being optional made an unrecognised key — `{ resourcez = …; }` — the same observation as a deliberate omission: read by nothing, contributing nothing, reported nowhere. The record is now closed to `resources` / `wiring` / `claims`, and anything else is refused by name with the key, the closed set, the kind and the path. The check quantifies over the record's KEYS rather than over the record, which is what leaves `{ }` returning — it is a legitimate empty answer and the live control the silent half above is measured against. The opposite ruling on `mkClaim`'s payload is not an inconsistency but the same principle read off a different reader: a payload is user-domain data this substrate carries opaquely and cannot enumerate, so an unrecognised key is its ordinary case; the result record is the substrate's own contract with the run, enumerable because the run enumerates it. Open where the reader is the caller, closed where the reader is this library.
...
| Export | Signature |
|---|---|
| `foldEquations` | `{ scope, parseParent, schedule, declaredDependencies, settings ? { } } -> ResolveCtx` |

`ResolveCtx` is exactly ten fields — `accessor`, `attributes`, `declaredDependencies`, `equations`, `eval`, `parseParent`, `roots`, `schedule`, `settings`, `trace` — and `ci/tests/fold-equations.nix` pins them as an exact set rather than by presence.
...
| Fixed-point / mutually recursive attribute | `circular { carrier = { bottom; leq; height; }; }` |
...
| `eval`'s argument pattern has no `...` — an unexpected key is a hard arity error `tryEval` cannot catch. The same closure is what makes `mkDecision` unable to carry a value-bearing field | `eval` in `lib/eval.nix`; `s.eval { roots = {}; attributes = {}; declaredDependencies = _: []; }` ⇒ `error: function 'eval' called with unexpected argument 'declaredDependencies'` (escapes `tryEval`). Positive control `s.eval { roots = {}; attributes = {}; prior = null; decision = s.coldDecision; }` ⇒ succeeded |
| The sealed relation is UNCHECKED and UNCONSULTED: nothing validates that a declared dependency names a real node, and neither evaluator consults the relation at all — so an attribute read that no declaration covers succeeds unremarked. Requiring the formal closes the *absent* case only; a *present but wrong* relation is still unenforced | `foldEquations` in `lib/fold-equations.nix`. A fold given `declaredDependencies = _: [ "no-such-node" ]` seals `accessor.dependencies "c"` ⇒ `[ "no-such-node" ]` and `trace.c.deps` ⇒ the same list, over a graph containing no such node; in the same eval `eval.get "c" "self-v"` ⇒ `2`, the read having consulted nothing. The `must over-declare` note at `accessor.dependencies` is a comment, not a contract |
...
| `circular` takes a REQUIRED carrier and refuses three different things by name — an absent or malformed declaration, a step the declared order does not ascend, and an ascent still running past the declared height. The last two used to be **one** message that named the iteration count and neither cause, so a monotone step on an unbounded carrier and an antitone step were indistinguishable from the outside | `circular` in `lib/resolve.nix`. `f = _: _: prev: prev + 1` under `height = 3` ⇒ *"still ascending after 4 steps, so the declared height of 3 is exceeded"*; the 2-cycle `if prev == 0 then 1 else 0` under the same carrier ⇒ *"took a step its declared order does not ascend, at iteration 1"*. Positive controls in the same suites: an ascending step converges at exactly `height + 1` evaluations, and a quotient carrier converges on the class. Tests: `circular` (`ci/tests/circular.nix`) for the booleans and the clean paths, `circular-refusals` (`ci/tests-error.nix`) for which refusal fired |
| The step count in the height refusal is the run's OWN, not `height + 1` restated — and that is load-bearing rather than cosmetic | Measured: with the bound deliberately loosened to `height + 50` the recited form stayed byte-identical while the loop ran fifty steps longer, so the message could not witness the bound it claims to derive. Reading `n + 1` makes the two numbers agree only when the derivation holds, and `test-an-exhausted-height-refutes-the-declaration` then fails on that seed |
...
| The library takes **four** inputs, three of them siblings: `graph` is gen-graph's `.lib`, `schema` is gen-schema's, and `identity` is gen-identity's — the mint, taken directly rather than through gen-schema, because a mint reached through a second library is a mint whose identity depends on that library's pin. `lib/graph.nix` (this library's algebraic constructors) is a DIFFERENT thing, bound as `algebraicGraph` inside `lib/default.nix` | `flake.nix` inputs are `gen-prelude` + `gen-graph` + `gen-schema`; `lib/default.nix` takes `{ prelude, graph, schema }`. The engine calls `graph.condensation program.dependency` and reads `.depth`; it partitions nothing itself. Of `schema` only `hashIdentity` is taken, and only `lib/mint.nix` receives it |
...
- **Söderberg & Hedin (2013), *Circular higher-order reference attribute grammars*** — `circular`'s declared carrier: *"a lattice of bounded height, that the semantic function is monotonic, and that a bottom value is provided as the starting point of the fixed-point iteration"* (§4.1, printed 311; the condition stated at §2.4, printed 305). Those three terms are the combinator's required declaration and what its derived `h + 1` iteration bound rests on.

- **Sloane et al. (2010), *Kiama: AG embedding*** — the `CachedAttribute` pattern realized as the co-located `_eval` cache; `paramAttr` (§3). ★ **`circular` is NOT claimed from this paper's §2.2, and the correction is recorded rather than quietly applied:** Sloane 2010 states no soundness condition anywhere, and its §2.2 is *Variable liveness* — an example, not a condition. What gen-scope inherited from it was the SIGNATURE, `circular (init : U) (f : T => U)` from the API table printed 211, bare `init` included; the citation travelled with the shape while the condition was never in the cited section to inherit. Both moved to the row above when the carrier landed. ★ **Collection attributes are NOT claimed from this paper and §7 is not a mechanism section**: §7 is *Conclusion and Future Work* and its whole content on them is "we are adding collection attributes [4,15]" — Kiama's future work. Its [15], Magnusson, Ekman & Hedin, is not held here, so the mechanism is claimed from nothing. `collectionAttr` is a **gather** and runs the opposite way round from an AG collection attribute: the collecting node enumerates its own sources through `traverse`, so a contributor contributes by being in reach, never by declaring it (README, `collectionAttr`; the surface table below files it as "Gather over …" for the same reason).
