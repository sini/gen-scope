...
- **Sloane et al. (2010), *Kiama: AG embedding*** — the `CachedAttribute` pattern realized as the co-located `_eval` cache; `paramAttr` (§3); `circular` fixed-point attributes (§2.2). ★ **Collection attributes are NOT claimed from this paper and §7 is not a mechanism section**: §7 is *Conclusion and Future Work* and its whole content on them is "we are adding collection attributes [4,15]" — Kiama's future work. Its [15], Magnusson, Ekman & Hedin, is not held here, so the mechanism is claimed from nothing. `collectionAttr` is a **gather** and runs the opposite way round from an AG collection attribute: the collecting node enumerates its own sources through `traverse`, so a contributor contributes by being in reach, never by declaring it (README, `collectionAttr`; the surface table below files it as "Gather over …" for the same reason).
