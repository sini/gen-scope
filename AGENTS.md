...
`lib/default.nix` is one flat merge — `algebraicGraph // buildNodes // queries // resolve // structural // interface // eval // program // leastModel // wellFounded // acceptance // engine // stratify // mint // folds // cascade`. All 89 exports sit at top level; there is no nested namespace and no key is shadowed (see traps). ★ In that merge `algebraicGraph` is this library's OWN `lib/graph.nix`; the parameter named `graph` is the **gen-graph library**, and the two are different things. `schema` is the third parameter and is bound to ONE function: `lib/default.nix` passes `hashIdentity` into `lib/mint.nix` and nothing else of that library reaches any module here.
...
- **No cascade rewire.** `resolveClaims` / `wiringFor` / `spliceWiring` run their own bounded loop over their own subject; `lib/mint.nix` names none of them and `lib/cascade.nix` names nothing of it. Two constructions sharing a library, not a pipeline.
