...
- **The edge set** — `foldEquations`' `declaredEdges` formal, sealed on the context and published as `accessor.edges`. This is an EDGE declaration, which is a different thing from a read declaration: it is what the gate runs over, complete at registration.
- **The trace** — `ctx.trace.<id>.deps`, derived per node as `accessor.edges id` at seal time. It duplicates nothing: it is the edge set, indexed. Tests: `test-trace-deps-are-the-accessor-edges`, `test-trace-deps-literal`, `test-trace-deps-empty-where-no-edge-is-declared` (`ci/tests/fold-equations.nix`).
- **Evaluation order** — governed by neither. `eval` schedules nothing; Nix's laziness is the scheduler.
- **Memoization** — governed by neither. Neither the edge set nor the trace runs through `get`, so neither reads nor populates any `_eval` cache.
...
- **What the evaluators accept** — neither takes a `declaredEdges` argument (both patterns are closed; passing one is an arity error). The edge set reaches the seal through `foldEquations` and nowhere else.

★ **WHAT IS NOT CONTRACTED, stated because the law names it as the discriminator.** `declaredEdges` defaults to `_: [ ]` and no codomain contract refuses a body that reads an edge it never declared; `accessor.edges`' own comment says the caller "must over-declare" and nothing enforces it. The edge set is therefore complete at registration in FORM and unenforced in FACT, and a gate trusting it inherits that gap.
...
| Read a node's dependency edges | `ctx.accessor.edges id`, or `ctx.trace.<id>.deps` for the same list indexed — both off `foldEquations`' seal. There is no projection between them and the edge set |
...
| The sealed edge set is UNCHECKED and UNCONSULTED: nothing validates that a declared edge names a real node, and neither evaluator consults the edge set at all — so an attribute read that no edge declares succeeds unremarked | `foldEquations` in `lib/fold-equations.nix`. A fold given `declaredEdges = _: [ "no-such-node" ]` seals `accessor.edges "c"` ⇒ `[ "no-such-node" ]` and `trace.c.deps` ⇒ the same list, over a graph containing no such node; in the same eval `eval.get "c" "self-v"` ⇒ `2`, the read having consulted nothing. The `must over-declare` note at `accessor.edges` is a comment, not a contract |
