...
#                              never had and which `genericClosure` replaced. `collectionAttr*` reads a
#                              CONSTANT here and that is the arm telling the truth, not a flat
#                              instrument; only `inheritAll*` is asserted on this axis.
#
...
# nothing, so a run in which that happens is REFUSED. `collectionAttr-defect` reaches the prior fold
...
arms="collectionAttr collectionAttr-defect inheritAll inheritAll-defect"
axes="list updates"
...
family() { # the shipped arm of this arm's family
  case $1 in
    collectionAttr*) echo collectionAttr ;;
    inheritAll*) echo inheritAll ;;
  esac
}

printf '%-22s %6s %3s %15s %22s  %s\n' arm n rc list.elements nrOpUpdateValuesCopied digest
for arm in $arms; do
  for n in $sizes; do
    rm -f "$tmp/stats.json"
...
for arm in $arms; do
  for axis in $axes; do
    ...
    overruns=0
    ratios=0
    # ON `list` ONLY. `collectionAttr*` allocates a CONSTANT number of set elements because it carries
    ...
    # budget on `list`; only `inheritAll` is held on `updates` — `collectionAttr` has no `//` to hold.
    case "$arm:$axis" in
      collectionAttr:list | inheritAll:list | inheritAll:updates)
        [ "$overruns" -eq 0 ] || refuse "$arm $axis overran the budget on $overruns of $ratios doublings"
        ;;
      collectionAttr-defect:list | inheritAll-defect:list | inheritAll-defect:updates)
        [ "$ratios" -gt 0 ] ||
